...
function novalib:MakeWindow(Configs)
	...
	MainCorner = Make("Corner", MainFrame)
	MakeDrag(MainFrame)
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			novalib.Tabs[TabSelect].func:Enable()
		else
			for _, Tab in pairs(novalib.Tabs) do
...
novalib:Notify({
    Title = "¡Bienvenido!",
    Content = "Has cargado Nova Lib correctamente",
    Icon = "check",
    Duration = 5
})

-- Notificación con imagen de fondo
novalib:Notify({
    Title = "Especial",
...
local notif = novalib:Notify({
    Title = "Atención",
    Content = "Mensaje permanente",
    Icon = "alert",
    Duration = 0
})
